...
function markAllMessagesRead() {
  ...
  .then(data => {
    if (data.status === "ok") {
      ...
      document.querySelectorAll('.msg-item .panel-dot').forEach(d => d.remove());
    }
  })
...
function markMsgRead(event, el) {
  ...
  setTimeout(() => { el.remove(); updateMsgCount(); }, 250);
}

function updateMsgCount() {
  const remaining = document.querySelectorAll('.msg-item').length;
  const badge = document.getElementById('msgBadge');
  const label = document.getElementById('msgCountLabel');
  if (remaining === 0) {
    ...
    if (list) list.innerHTML = '<div class="panel-empty"><div>📭</div>No new messages</div>';
  } else {
...
function markNotifRead(event, el) {
  el.classList.add('fade-out');
  setTimeout(() => { el.remove(); updateNotifCount(); }, 250);
}

function updateNotifCount() {
  const remaining = document.querySelectorAll('.notif-ticket-item').length;
  ...
  if (remaining === 0) {
    ...
    if (list) list.innerHTML = '<div class="panel-empty"><div>🔕</div>No new notifications</div>';
  } else {
    if (badge) badge.textContent = remaining;
